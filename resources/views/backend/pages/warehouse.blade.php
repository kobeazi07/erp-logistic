                                ...
                                <div class="modal-dialog modal-lg">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <form id="formwarehouse" enctype="multipart/form-data">
                                                @csrf
                                                {{-- <div class="form-group">
                                                    <label for="exampleFormControlInput1">Kode warehouse</label>
                                                    <input type="text" class="form-control" name="kode_warehouse"
                                                        id="exampleFormControlInput1" placeholder="masukkan Kode warehouse">
                                                </div> --}}
                                                <div class="form-group">
                                                    <label for="exampleFormControlInput1">Nama warehouse</label>
                                                    <input type="text" class="form-control" name="warehouse_name"
                                                        id="exampleFormControlInput1" placeholder="masukkan nama warehouse">
                                                </div>
                                                <div class="form-group">
                                                    <label>Cabang</label>
                                                    <select id="id_kat_program" name = "cabang_id" class="form-control">
                                                        <option selected disabled riquired>Choose...</option>
                                                        @foreach ($cabang as $cabangs)
                                                            <option value=" {{ $cabangs->id }}">
                                                                {{ $cabangs->alamat }} </option>
                                                        @endforeach
                                ...
                                <table id="example" class="display" style="min-width: 845px">
                                    ...
                                    <tbody>
                                        @foreach ($warehouse as $key => $warehouse)
                                            <tr>
                                                <td>{{ $key + 1 }}</td>
                                                <td>{{ $warehouse->prefix . str_pad($warehouse->number, 4, '0', STR_PAD_LEFT) }}
                                                </td>
                                                <td>{{ $warehouse->warehouse_name }}</td>
                                                <td>{{ $warehouse->cabang->alamat }}</td>
                                                <td>
                                                    {{-- detail --}}
                                                    <a href="{{ route('HalamanDWarehouse', $warehouse->id) }}">
                                                    ...
                                                    {{-- modal edit --}}
                                                    <!-- Modal -->
                                                    <div class="modal fade" id="Edit-{{ $warehouse->id }}"
                                                                    ...
                                                                    <div class="modal-body">
                                                                        {{-- <div class="form-group">
                                                                            ...
                                                                            <input type="text" class="form-control"
                                                                                ...
                                                                                value="{{ $warehouse->kode_warehouse }}">
                                                                        </div> --}}
                                                                        <div class="form-group">
                                                                        ...
                                                                        <div class="form-group">
                                                                            <label>Cabang</label>
                                                                            <select id="id_kat_program" name = "cabang_id"
                                                                                class="form-control">
                                                                                <option selected disabled riquired>Choose...
                                                                                </option>
                                                                                @foreach ($cabang as $cabangss)
                                                                                    <option value=" {{ $cabangss->id }}"
                                                                                        @selected($cabangss->id == $warehouse->cabang_id)>
                                                                                        {{ $cabangs->alamat }} </option>
                                                                                @endforeach
                                                    ...
                                                    <form action="{{ route('Warehouse.destroy', $warehouse->id) }}"
                                                        method="POST" class="form-delete-warehouse mt-3">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
