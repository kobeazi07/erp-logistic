...
@section('konten')
    ...
    ***********************************-->
                                ...
                                <div class="modal-dialog modal-lg">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <form id="formjabatan" enctype="multipart/form-data">
                                                @csrf
                                                {{-- <div class="form-group">
                                                    <label for="exampleFormControlInput1">Kode jabatan</label>
                                                    <input type="text" class="form-control" name="kode_jabatan"
                                                        id="exampleFormControlInput1" placeholder="masukkan Kode jabatan">
                                                </div> --}}

                                                <div class="form-group">
                                                    <label for="exampleFormControlInput1">Nama jabatan</label>
                                                    <input type="text" class="form-control" name="name_jabatan"
                                                        id="exampleFormControlInput1" placeholder="masukkan nama jabatan">
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-secondary"
                                                        data-dismiss="modal">Close</button>
                                                    <button type="button" class="btn btn-primary" id="btnSavejabatan">Save
                                                        changes</button>
                                                </div>
                                            </form>
                                        </div>
                                ...
                                <table id="example" class="display" style="min-width: 845px">
                                    ...
                                    <tbody>
                                        @foreach ($jabatan as $key => $jabatan)
                                            <tr>
                                                <td>{{ $key + 1 }}</td>
                                                <td>{{ $jabatan->prefix . str_pad($jabatan->number, 4, '0', STR_PAD_LEFT) }}
                                                </td>
                                                <td>{{ $jabatan->name_jabatan }}</td>
                                                <td>
                                                    {{-- edit  --}}
                                                    <button type="button" class="btn btn-warning" data-toggle="modal"
                                                        data-target="#Edit-{{ $jabatan->id }}">
                                                    ...
                                                    {{-- modal edit --}}
                                                    <!-- Modal -->
                                                    <div class="modal fade" id="Edit-{{ $jabatan->id }}"
                                                        ...
                                                        <div class="modal-dialog modal-lg">
                                                            <div class="modal-content">
                                                                ...
                                                                <form class="editformjabatan" data-id="{{ $jabatan->id }}"
                                                    ...
                                                    <form action="{{ route('Jabatan.destroy', $jabatan->id) }}"
                                                        method="POST" class="form-delete-jabatan mt-3">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
    ...
    <script>
        $('#btnSavejabatan').on('click', function() {
            let form = document.getElementById('formjabatan');
            let formData = new FormData(form);


            $.ajax({
                url: "{{ route('Tambah_Jabatan') }}",
                type: "POST",
                data: formData,
                processData: false,
                contentType: false,
        ...
        // edit
        $(document).on('submit', '.editformjabatan', function(e) {
            e.preventDefault();
        ...
        // delete
        $(document).on('submit', '.form-delete-jabatan', function(e) {
            e.preventDefault();
